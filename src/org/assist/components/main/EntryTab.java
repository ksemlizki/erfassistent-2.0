...
import org.assist.components.common.*;
...
public class EntryTab extends JPanel {
    ...
    public void specialReplace() {
        ...
        final SpecialReplaceDialog specialReplaceDialog = new SpecialReplaceDialog(MainFrame.getInstance());
        specialReplaceDialog.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosed(WindowEvent e) {
                if (specialReplaceDialog.isResult()) {
                    String searchText = specialReplaceDialog.getSearchTextTF().getText();
                    int count = Integer.parseInt(specialReplaceDialog.getCountTF().getText());

                    processSpecialReplace(searchText, count);
                }
            }
        });


        specialReplaceDialog.setVisible(true);


    }

    protected void processSpecialReplace(String searchText, int count) {
        ArrayList<Data> allData = getAllData();
        for (int i = 0; i < allData.size(); i++) {
            Data data = allData.get(i);
            String text = data.getText();
            text = replace(text, searchText, count);
            data.setText(text);
        }
    ...
    private String replace(String text, String searchText, int count) {
